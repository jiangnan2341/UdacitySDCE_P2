...
def show_pcl(pcl):
    ...
    pcl_vis = o3d.visualization.VisualizerWithKeyCallback()
    pcl_vis.create_window(window_name='Open3D', width=1280, height=720, left=50, top=50, visible=True)
    def key_callback(pcl_vis):
        pcl_vis.close()
    pcl_vis.register_key_callback(262, key_callback)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    intensity = (256 * intensity_map).astype(np.uint8) # normalized 8-bit intensity map
    '''
    while(1):
        cv2.imshow('img_intensity', intensity)
        if cv2.waitKey(10) & 0xff ==27:
            break
    cv2.destroyAllWindows()
    '''
    #cv2.imwrite('./results/bev_intensity_channel.png', intensity)
    ...
    height_map[np.int_(lidar_pcl_top[:, 0]),
               np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 2] / float(configs.lim_z[1] - configs.lim_z[0])

    ## step 3 : temporarily visualize the intensity map using OpenCV to make sure that vehicles separate well from the background
    height = (256 * height_map).astype(np.uint8) # normalized 8-bit height map
    '''
    while(1):
        cv2.imshow('img_height', height)
        if cv2.waitKey(10) & 0xff ==27:
            break
    cv2.destroyAllWindows()
    '''
    #cv2.imwrite('./results/bev_height_channel.png', height)
    #######
    ####### ID_S2_EX3 END #######       
